...
- Sentence playback with word-level highlighting, 0.5×–2× speed, and mid-document
  voice switching across 26 voices in 12 languages.
...
import merges by content hash, keeping the furthest reading position.
...
test runner — no framework, one process per file (`lib/idb` caches its connection).
It covers the backup format, the v1→v2 database upgrade, cache eviction, the
service worker, rate limiting, and find.
